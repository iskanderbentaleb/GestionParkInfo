...
<?php
$title_page = "Modification d'un décharge";
...
<form class="max-w-sm  mx-auto mt-10 " action="index.php?page=decharge&action=update" method="post">
    

<input name="CodeDech" value="<?= $data['DechargeInfo'][0]->CodeDech ?>" type="text" required hidden>
  ...
  <select id="Type" name="Type" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500 dark:shadow-sm-light" required>
    ...
    <option value="Decharge"
    <?php if($data['DechargeInfo'][0]->Type == 'Decharge'){echo "selected";} ?>
    >Decharge</option>

    <option value="Retour"
    <?php if($data['DechargeInfo'][0]->Type == 'Retour'){echo "selected";} ?>
    >Retour</option>
  ...
  <?php
    ...
    <?php if($utilisateurs['CodeUt'] == $data['DechargeInfo'][0]->CodeUtilisateur){echo "selected";} ?>
    >
    <?= $utilisateurs["Email"] ; ?>
    </option>
    <?php endforeach; ?>
