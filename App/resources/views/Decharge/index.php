...
<div x-data="MatreilsFilter()" x-init="
    fetch('http://localhost/GESTIONPARKINFO/App/Controllers/Api/DechargeApi.php')
    .then(response => response.json())
    .then(response => {
        decharges = response.data;
...
<a href="index.php?page=decharge&action=create">
        ...
        <table class="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400 rounded-xl ">
            ...
            <thead class="text-xs text-gray-700 uppercase bg-gray-900 text-white ">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Pour</th>
            ...
            <tbody>
                ...
                <template x-for="decharges in filteredDecharges" :key="decharges.CodeDech">
                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 ">
                        <td class="px-6 py-4" x-text="decharges.CodeDech"></td>
                        <td class="px-6 py-4" x-text="decharges.Date"></td>
                        <td class="px-6 py-4" x-text="decharges.Dechtype" x-bind:class="{ 'text-green-500': decharges.Dechtype === 'Decharge', 'text-red-500': decharges.Dechtype !== 'Decharge' }"></td>

                        <td class="px-6 py-4" x-text="decharges.utNom + ' ' + decharges.utPrenom"></td>
                        <td class="p-2 flex">
                                ...
                                <button 
                                    ...
                                    x-on:click="openModal(decharges.CodeDech)">
                                    <img src="../images/moree.svg" width="22">
                                </button>



                            <a x-bind:href="'index.php?page=decharge&CodeDech=' + decharges.CodeDech + '&action=edit'" class="font-medium mr-5 bg-gray-100 p-4 rounded-xl text-blue-700 dark:text-blue-500 hover:underline">
                                <img src="../images/update.svg" width="20" alt="update">
                            </a>
                            <button x-on:click="DeleteRecordValidation('index.php?page=decharge&CodeDech=' + decharges.CodeDech + '&action=destroy')" class="font-medium mr-5 bg-gray-100 p-4 rounded-xl text-blue-700 dark:text-blue-500 hover:underline">
...
<script>
    ...
    function MatreilsFilter() {
        return {
            ...
            get filteredDecharges() {
                if (!this.searchQuery.trim()) {
                    return this.decharges;
                } else {
                    const query = new RegExp(this.searchQuery.trim(), 'i'); // 'i' for case-insensitive
                    return this.decharges.filter(Decharge =>
                        query.test(Decharge.CodeDech ) ||
                        query.test(Decharge.Date) ||
                        query.test(Decharge.Type) ||
                        query.test(Decharge.CodeUtilisateur)
                    );
                }
            },
            ...
            openModal(CodeDech) {
                ...
                const filteredMatreilADecharges = this.MatreilADecharges.filter(MatreilADecharge =>
                    MatreilADecharge.CodeDech === CodeDech
                );
                ...
                $(function() {
                    $("#dialog-message").html(tableHTML).dialog({
                        modal: true,
                        title: "Contenu de Décharge",
                        width: 780, // Set the width
                        height: 600, // Set the height
                        buttons: {
                            Imprimer: function() {
                            window.open("index.php?page=decharge&action=print&dech=" + CodeDech, "_blank");
                            },
                            Fermer: function() {
                                $(this).dialog("close");
                            }
