...
<form class="mx-auto mt-10 p-8" action="index.php?page=commandes&action=update" method="post">
  ...
  <input type="text" name="codeCommande" value="<?= $data['commandeInfo']['getCommandeInfo'][0]->CodeCom; ?>" required hidden>
  ...
  <div class="mr-5" style="width: 33%;">
      ...
      <input type="text" value="<?= $data['commandeInfo']['getCommandeInfo'][0]->CodeCom; ?>" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500 dark:shadow-sm-light" required disabled>
  </div>
      ...
      <select name="typeCommande" id="typeCommande" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500 dark:shadow-sm-light" required>
          <option value="" selected disabled>sélectionner le type de commande</option>
            ...
            foreach($data['CommandeTypes'] as $dataCommandeTypes) : ?>
                ...
                <?php if($dataCommandeTypes["CodeType"] == $data['commandeInfo']['getCommandeInfo'][0]->CodeCommandeType){echo "selected";} ?>
                > 
  ...
  <label for="fournisseur" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fournisseur</label>
    ...
    foreach($data['fournisseurs'] as $datafournisseur) : ?>
        ...
        <?php if($datafournisseur["CodeFour"] == $data['commandeInfo']['getCommandeInfo'][0]->CodeFournisseur){echo "selected";} ?>
        > 
  ...
  <div style="width: 33%;">
    ...
    <input type="date" value="<?= $data['commandeInfo']['getCommandeInfo'][0]->Date; ?>" name="dateCommande" id="dateCommande" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500 dark:shadow-sm-light" required disabled/>
  </div>
...
<div id="content-container">
  ...
  <?php
    $totalRecords = count($data['commandeInfo']['getCommandeContenue']);
    $currentRecordIndex = 0;
    foreach($data['commandeInfo']['getCommandeContenue'] as $dataCommandeContenu) : 
  ...
  <div class="content-item flex items-center mb-4">
    ...
    <select name="materielType[]" class="materielType form-select mr-2 flex-1 py-2 px-4 rounded-lg focus:outline-none" required >
        ...
        foreach($data['MatreilTypes'] as $dataMatreilTypes) : ?>
            ...
            <?php if($dataMatreilTypes["CodeType"] == $dataCommandeContenu->CodeType){echo "selected";} ?>
            > 
    ...
    <select name="marque[]" class="marque form-select mr-2 flex-1 py-2 px-4 rounded-lg focus:outline-none" required>
      <option value="" selected disabled>Choisissez un marque</option>
        ...
        foreach($data['Marques'] as $dataMarques) : ?>
            ...
            <?php if($dataMarques["CodeMrq"] == $dataCommandeContenu->CodeMrq){echo "selected";} ?>
            > 
    ...
    <!-- Qty input -->
    <input type="number" min="1" value="<?php echo $dataCommandeContenu->Qty; ?>" name="qty[]" class="qty form-input flex-1 py-2 px-4 rounded-lg focus:outline-none" placeholder="Qty" required>


    
    <?php if ($currentRecordIndex === $totalRecords) : ?>
      <button type="button" id="add-content-item" class="add-content-item bg-blue-500 text-white font-medium rounded-lg text-sm px-3 py-2 ml-2" >Ajouter</button>
      <button type="button" id="delete-button" class="delete-button bg-red-500 text-white font-medium rounded-lg text-sm px-3 py-2 ml-2">x</button>
    <?php endif; ?>
  ...
  <button style="background-color: #F08100" type="submit" class="text-white font-medium rounded-lg text-sm px-5 py-2.5 text-center">Modifier</button>
  </div>
...
<script>
  ...
  function deleteContentItem() {
    // Select the last content item
    var contentItems = document.querySelectorAll(".content-item");
    var lastContentItem = contentItems[contentItems.length - 1];


    // Remove the last content item
    lastContentItem.parentNode.removeChild(lastContentItem);



    
    
    // Show the "Ajouter" button for the previous content item
    var previousContentItem = contentItems[contentItems.length - 2];
      previousContentItem.appendChild(ajouterButton);
      previousContentItem.appendChild(deleteButton);
      changeSelectMarque();


    
    // Hide the "x" button if there are less than 3 content items
    // var deleteButton = document.getElementById("delete-button");
